...
from app.core.config import DOCUMENT_TYPE_CONFIG
...
class DocumentProcessor:
    ...
    def _process_text(self, file_path: Path) -> List[Document]:
        """Process text file (website content, policy document, or plain text)."""
        documents = []
    ...
    def _process_rtf(self, file_path: Path) -> List[Document]:
        """Process RTF file."""
        documents = []
        ...
        try:
            ...
            # Create document with converted text content
            documents.append(
                Document(
                    text=plain_text,
                    metadata={
            ...
            logger.info(f"Processed RTF file: {file_path.name}")
            
        except Exception as e:
            logger.error(f"Error processing RTF file: {e}")
            raise ValueError(f"Failed to process RTF file: {str(e)}")
        
        return documents
        
    def _process_legacy_text(self, file_path: Path) -> List[Document]:

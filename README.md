...
**POST** `/chatbot/api/upload`

Upload and index a document into the RAG system.

```bash
# Upload FAQ Excel file
curl -X POST "http://localhost:8000/chatbot/api/upload" \
  -F "file=@faq.xlsx" \
  -F "document_type=faq"
...
```bash
curl "http://localhost:8000/chatbot/api/health"
```
...
```bash
curl "http://localhost:8000/chatbot/api/stats"
```
...
with open('test_faq.xlsx', 'rb') as f:
    response = requests.post(
        'http://localhost:8000/chatbot/api/upload',
        files={'file': f},
...
# Query
response = requests.post(
    'http://localhost:8000/chatbot/api/query',
    json={
